...
const TablaTicket = ({ rows }: { rows: TicketRow[] }) => (
  <table style={{ width: "auto", borderCollapse: "collapse", fontSize: "13pt", fontWeight: 700, fontFamily: "Arial, Helvetica, sans-serif", marginTop: 4 }}>
    <thead>
      <tr>
        <th style={{ border: "1px solid #888", padding: "6px 10px", textAlign: "left", whiteSpace: "nowrap", backgroundColor: "#ececec" }}>Clave</th>
        <th style={{ border: "1px solid #888", padding: "6px 10px", textAlign: "left", backgroundColor: "#ececec" }}>Producto</th>
        <th style={{ border: "1px solid #888", padding: "6px 10px", textAlign: "center", whiteSpace: "nowrap", backgroundColor: "#ececec" }}>Cant.</th>
        <th style={{ border: "1px solid #888", padding: "6px 10px", textAlign: "center", whiteSpace: "nowrap", minWidth: 80, backgroundColor: "#ececec" }}>Revisado</th>
      </tr>
    </thead>
    <tbody>
      {rows.map((p, i) => (
        <tr key={i} style={{ backgroundColor: i % 2 === 1 ? "#f7f7f7" : "#ffffff" }}>
          <td style={{ border: "1px solid #888", padding: "6px 10px", whiteSpace: "nowrap" }}>{p.clave}</td>
...
const EntradaMercancia = () => {
  ...
  const buscarHistorial = async (
    ...
    try {
      ...
      const res = await axios.get(
        `https://ato-appservidor-nvxt.onrender.com/inventario/inventario/entradas`,
        { params, ...config }
      );
  ...
  const obtenerEncargado = async (moduloId: number) => {
    ...
    try {
      const res = await axios.get(
        `https://ato-appservidor-nvxt.onrender.com/modulos/${moduloId}/encargado`,
        config
      );
  ...
  const obtenerExistenciaModulo = async (clave: string) => {
    try {
      const res = await axios.get(
        `https://ato-appservidor-nvxt.onrender.com/inventario/inventario/modulo/${moduloSeleccionado}/existencia`,
        { params: { clave }, ...config }
      );
  ...
  const guardarEntradaMercancia = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://ato-appservidor-nvxt.onrender.com/inventario/inventario/entrada_mercancia`,
        payload,
        config
      );
      if (res.data.ok) {
  ...
  const imprimirEntradaDetalle = async () => {
    ...
    try {
      const res = await axios.get(
        `https://ato-appservidor-nvxt.onrender.com/modulos/${entradaDetalle.modulo_id}/encargado`,
        config
      );
      enc = res.data ?? null;
    } catch {
  ...
  const handleEliminar = async () => {
    ...
    try {
      await axios.delete(
        `https://ato-appservidor-nvxt.onrender.com/inventario/inventario/entradas/${entradaDetalle.id}`,
        config
      );
  ...
  const handleGuardarEdicion = async () => {
    ...
    const url = `https://ato-appservidor-nvxt.onrender.com/inventario/inventario/entradas/${entradaDetalle.id}`;
    try {
